...
let parse_opt error active errflag s =
  ...
  let set i =
    ...
    else flags := Bitfield.set i !flags
  in
  let clear i =
    if i = 3 then set_alert ~error:errflag ~enable:false "deprecated"
    else flags := Bitfield.unset i !flags
  in
  let set_all i =
    if i = 3 then begin
      ...
      active := Bitfield.set i !active;
      error := Bitfield.set i !error
    end
  in
...
let parse_options errflag s =
  ...
  current := { (!current) with error = !error; active = !active}
